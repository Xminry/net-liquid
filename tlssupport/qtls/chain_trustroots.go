...
type ChainTrustRoots struct {
	lock               sync.Mutex
	trustRoots         map[string]*qx509.CertPool
	trustIntermediates map[string]*qx509.CertPool
}

// NewChainTrustRoots create a new ChainTrustRoots instance.
func NewChainTrustRoots() *ChainTrustRoots {
	return &ChainTrustRoots{trustRoots: make(map[string]*qx509.CertPool), trustIntermediates: make(map[string]*qx509.CertPool)}
}
...
func (ctr *ChainTrustRoots) AddRoot(chainId string, root *qx509.Certificate) {
	...
	cp, ok := ctr.trustRoots[chainId]
	if !ok {
		ctr.trustRoots[chainId] = qx509.NewCertPool()
...
func (ctr *ChainTrustRoots) AddIntermediates(chainId string, intermediates *qx509.Certificate) {
	...
	cp, ok := ctr.trustIntermediates[chainId]
	if !ok {
		ctr.trustIntermediates[chainId] = qx509.NewCertPool()
...
func (ctr *ChainTrustRoots) VerifyCert(cert *qx509.Certificate) ([]string, error) {
	...
	for chainId, certPool := range ctr.trustRoots {
		vo := qx509.VerifyOptions{Roots: certPool}
